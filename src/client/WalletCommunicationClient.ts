...
export class WalletCommunicationClient {
  ...
  constructor(public privateSeed = 'secret', public replicationCount = 3) {}
  ...
  public getRelayServer(
    publicKeyHash: string = Buffer.from(sodium.crypto_generichash(32, this.keyPair.publicKey)).toString('hex'),
    nonce: string = ''
  ): string {
    return this.KNOWN_RELAY_SERVERS.reduce((prev, curr) => {
      ...
      return this.getAbsoluteBigIntDifference(publicKeyHash, prevRelayServerHash) <
        this.getAbsoluteBigIntDifference(publicKeyHash, currRelayServerHash)
        ? prev
        : curr
    })
  }
  ...
  public async start() {
    ...
    console.log(
      ...
      'on',
      this.getRelayServer(Buffer.from(sodium.crypto_generichash(32, this.keyPair.publicKey)).toString('hex'), '0')
    )
    ...
    for (let client of this.clients) {
      client.on('event', (event: any) => {
        if (
          ...
          event
            .getSender()
            .startsWith('@' + Buffer.from(sodium.crypto_generichash(32, Buffer.from(senderPublicKey, 'hex'))).toString('hex'))
        ) {
