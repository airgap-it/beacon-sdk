...
/**
 *  @category Error
 *  @description Error that indicates the combinatison pkh-network is not part of the active session
 */
export class InvalidNetworkOrAccount extends Error {
  ...
  constructor(
    ...
    super(
      `No permission. The combinatison "${network}" and "${pkh}" is not part of the active session.`
    )
  }
}
