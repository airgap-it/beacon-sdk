...
const Toast: React.FC<ToastProps> = (props: ToastProps) => {
  ...
  const hasWalletObject =
    props.label.includes("{{wallet}}") && props.walletInfo;
  ...
  const onMouseDownHandler = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (
      target.className !== "toast-header" &&
      target.parentElement?.className !== "toast-header"
    ) {
      return;
    }
  ...
  return (
    ...
    >
      <div className="toast-header">
        <Loader />
        {hasWalletObject && props.walletInfo && (
          <>{parseWallet(props.label, props.walletInfo)}</>
        )}
        ...
        <div className="toast-body">
          {props.actions.map((action, index) => (
            <div key={index} className="toast-action-wrapper">
              <p
                className={`toast-action-label ${
                  action.isBold ? "toast-action-bold" : ""
                }`}
              >
                {action.text}
              </p>
