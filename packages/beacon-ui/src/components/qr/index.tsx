...
const QR: React.FC<QRProps> = (props: QRProps) => {
  ...
  return (
    ...
    >
      <Grid2 container justifyContent={'center'} alignItems={'center'} flexDirection={'column'}>
        ...
        {props.isMobile && (
          <span>
            ...
            {props.onClickLearnMore && <span onClick={props.onClickLearnMore}>Learn more</span>}
          </span>
        )}
      ...
      <Grid2 container onClick={handleCopyClipboard}>
        <div style={{ cursor: 'pointer' }} dangerouslySetInnerHTML={{ __html: qrSVG }} />
      </Grid2>
      <Grid2
        container
        alignItems={'baseline'}
        gap={'5px'}
        sx={copied ? undefined : { cursor: 'pointer' }}
      >
        {copied ? (
          <>
            <svg
