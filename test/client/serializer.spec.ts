...
describe(`serializer - Custom Tests`, () => {
  ...
  testCases.forEach(testCase => {
    it(`should serialize "${testCase.output}"`, () => {
      const output = serializer.serialize(testCase.input)
      expect(output).to.deep.equal(testCase.output)
    })

    it(`should deserialize "${testCase.output}"`, () => {
      if (testCase.reconstructed) {
        const reconstructed = serializer.deserialize(testCase.output)
